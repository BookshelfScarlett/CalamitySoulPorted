...
namespace CalamitySoulPorted.SoulProjectiles.Ranged.BlissfulBombardierRebornProjectiles
{
    public class BlissfulBombardierRebornNuke: ModProjectile, ILocalizedModType
    {
        //删除残影
        public new string LocalizationCategory => SoulGlobalProjectiles.RangedPath;
        ...
        public const float IsFlying = 0f;
        public const float IsAngleToTarget = 1f;
        public const float IsRushing = 2f;
        //从原灾抄过来的
        ...
        private void DoRushing(NPC target)
        {
            if (target is null)
                return;
            Projectile.HomeInOnTarget(target, 1800f, 30f, 0f, 2, default, default, true);
            SpawnDamageDust();
        }
        ...
        {
            SoundEngine.PlaySound(SoulSoundID.SoundBomb, Projectile.position);
            UpdateProjectile();
            OnKillDust();
            OnKillGore();
            BreakTiles();
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<BlissfulBombardierRebornNukeDust>(), (int)(Projectile.damage * 0.25), Projectile.knockBack, Projectile.owner, 0f, 2f);
        }
        ...
        private void SpawnDamageDust()
        {
            ...
            if (SummonFlaresTimer % 12 == 0)
                if (Projectile.owner == Main.myPlayer)
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position, Vector2.Zero, ModContent.ProjectileType<BlissfulBombardierRebornNukeDust>(), Projectile.damage/2, Projectile.knockBack, Projectile.owner, 0f, 0f);
        ...
        private void OnKillGore()
        {
            if (Main.netMode != NetmodeID.Server)
            {
                ...
                for (int goreIndex = 0; goreIndex < goreAmt; goreIndex++)
                {
                    ...
                    Mod mod = ModContent.GetInstance<CalamitySoulPorted>();
